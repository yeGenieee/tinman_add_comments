...
class TransactionSigner(object):
    def __init__(self, sign_transaction_exe=None, chain_id=None):
        if(chain_id is None): # chain_id가 없는 경우
...
# 캐시된 Dynamic global properties objects
...
def generate_blocks(steemd, args, cached_dgpo=None, now=None, produce_realtime=False):
    if args["count"] <= 0: # count가 0보다 작거나 같으면 생성될 블록이 없는 것 ==> 바로 리턴
        return

    miss_blocks = args.get("miss_blocks", 0) # miss_block

    if not produce_realtime: # produce_realtime이 False인 경우 ==> 이후에도 블록 생성 안함
...
def main(argv):
    ...
    parser.add_argument("--realtime", dest="realtime", action="store_true", help="Wait when asked to produce blocks in the future") # realtime은 이후에도 블록을 생성할 그것인지 결정하는 플래
    args = parser.parse_args(argv[1:])

    die_on_fail = False # die_on_fail?
    if args.fail_file == "-":
        fail_file = sys.stdout
    elif args.fail_file == "die": # fail 파일이 die인 경우
        fail_file = sys.stdout
        die_on_fail = True
    else:
    ...
    if args.chain_id != "": # -c 옵션에 해당하는 chain_id가 있음
        chain_id = hashlib.sha256(args.chain_id).digest()
    else: # 현재 tinman readme.md에 의해서는 -c 옵션이 없음
        chain_id = None

    signer = TransactionSigner(sign_transaction_exe=sign_transaction_exe, chain_id=chain_id) # sign_transaction , chain_id는 None (서명할 함수 지정)
    ...
    for line in input_file:
        ...
        try:
            if cmd == "wait_blocks": # wait_blocks로 시작하는 json
