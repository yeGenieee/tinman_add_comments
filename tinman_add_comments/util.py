...
def tag_escape_sequences(s, esc):
    ...
    # 짤린 문자열 s와 s가 esc 문자열로 둘러싸여있는지 아닌지 리턴 (둘러싸여있으면 True, 아니면 False)
    return zip(s.split(esc), itertools.cycle([False, True]))

# 정해진 size로 들어온 스트링(it) 자르고, size끼리 묶음
def batch(it, size=1):
    """
...
def action_to_str(action):
    ...
    JSON 형태가 아닌 string을 꺼냄 (Action으로 직렬화)
    """
    json_empty_esc = json.dumps(action, separators=(",", ":"), default=prockey.PubkeySerializer(esc=""), sort_keys=True)  # sort_keys가 True이므로 출력으로 나오는 딕셔너리는 키로 정렬됨
    # print("@json_empty_esc " + str(json_empty_esc))
    esc = find_non_substr(json_empty_esc, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
    # print("@esc " + str(esc))
    action[1]["esc"] = esc # json에 esc추가
    return json.dumps(action, separators=(",", ":"), default=prockey.PubkeySerializer(esc=esc), sort_keys=True) # esc가 무조건추가됨
